...
`summary.warn` 是完整的可操作警告总数。`summary.containedWarn` 是其子集，不是额外的计数桶。只有 `STALE_SEED`、`SEED_ERROR`、`STALE_CONTENT`、`COVERAGE_PARTIAL` 和 `COVERAGE_DEGRADED` 有资格受控，而且元数据必须证明记录数为正、不存在已知不可用情况，且不在有效宽限期内。受控警告不超过所有探测键的 3% 时，可用性保持 `HEALTHY`；缺失或不可用的数据、未知状态、`REDIS_PARTIAL`、`ROLLOUT_PENDING`、更广泛的影响和严重故障不适用此规则。 受控判断会独立检查所有必需的诊断证据，不受最终状态选择顺序影响。陈旧年龄未知、读取策略或缓存状态不兼容、实物价格背离警告以及 `CHINA_DEGRADED` 均不受控；汇总行数不能证明底层中国数据集仍然可用。
