...
| HTTP 状态 | 总体状态 | 含义 |
|-------------|---------------|---------|
| `200` | `HEALTHY` | 用户可见平台可用。所有检查正常，或少量受控警告仍提供可用的最后一次正常数据 |
...
`summary` 字段：`total` 是每个探测键的一个条目，并随面板增加而增长 —— 请以你自己响应中的数值为准，而非本页示例。`warn` 是完整的可操作警告总数，但**不包括**按需为空的键；这些键在 `onDemandWarn` 中单独显示。`containedWarn` 是 `warn` 的子集，不是额外的计数桶。只有 `STALE_SEED`、`SEED_ERROR`、`STALE_CONTENT`、`COVERAGE_PARTIAL` 和 `COVERAGE_DEGRADED` 有资格受控，而且元数据必须证明记录数为正、不存在已知不可用情况，且不在有效宽限期内。如果所有可操作警告都受控，且数量不超过所有探测键的 3%，可用性仍为 `HEALTHY`。 受控判断会独立检查所有必需的诊断证据，不受最终状态选择顺序影响。陈旧年龄未知、读取策略或缓存状态不兼容、实物价格背离警告以及 `CHINA_DEGRADED` 均不受控；汇总行数不能证明底层中国数据集仍然可用。

缺失或不可用的数据、未知警告状态、`REDIS_PARTIAL`、`ROLLOUT_PENDING`、超过 3% 的受控警告，以及所有严重故障都会影响可用性。`EMPTY_ON_DEMAND` 和待处理诊断保留现有语义。`staleContent` 统计所有 `STALE_CONTENT` 诊断。在数据源处于三小时的 `staleContentGraceUntil` 宽限期内时，该值可以大于 `warn`。数据源在截止时间前计入 `ok` 和可选的 `pending`。`rolloutPending` 是 `warn` 的一个**子集**。只有 `crit`（`EMPTY`/`EMPTY_DATA`）会驱动 `DEGRADED`/`UNHEALTHY`。
