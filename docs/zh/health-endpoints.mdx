...
`summary` 字段：`total` 是每个探测键的一个条目，并随面板增加而增长 —— 请以你自己响应中的数值为准，而非本页示例。`warn` 是完整的可操作警告总数，但**不包括**按需为空的键；这些键在 `onDemandWarn` 中单独显示。`containedWarn` 是 `warn` 的子集，不是额外的计数桶。只有 `STALE_SEED`、`SEED_ERROR`、`STALE_CONTENT`、`COVERAGE_PARTIAL`、`COVERAGE_DEGRADED` 和 `CHINA_DEGRADED` 有资格受控，而且元数据必须证明记录数为正、不存在已知不可用情况，且不在有效宽限期内。如果所有可操作警告都受控，且数量不超过所有探测键的 3%，可用性仍为 `HEALTHY`。
