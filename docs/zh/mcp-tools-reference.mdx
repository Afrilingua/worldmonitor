...
在依据空的 `events` 列表下结论之前，务必先读 `sources`。每个数据源都会报告自己的 `state`，只要其中任何一个不健康，`degraded` 即为 true，因此上游故障造成的沉默不会被误读为平静的一周。关键区别在于：`empty` 断言该数据源可达且确实没有内容，而 `unknown` 表示它没有返回任何内容、且本接口无法确认其上游是否正常——部分上游处理器对"失败"和"确实为空"的返回完全相同，因此无法确认的沉默会被如实命名，而不是默认为健康。`stale` 仍会贡献事件；`failed` 与 `unavailable` 不贡献任何事件。本接口上有两个数据源恒为 `unavailable`：军用船舶通道来自浏览器内的实时 AIS 流，服务端没有对应实现；中东打击追踪已于 2026 年 7 月退役。
...
| `window_hours` | integer | 否 | 回溯窗口小时数，1-168。默认 168（7 天），与界面一致。上游报道查询固定为 7 天，因此更大的取值会被拒绝，而不是静默返回同一批事件。 |
| `limit` | integer | 否 | 返回的时间线事件上限，保留最新的若干条。1-500，默认 200。 |
