...
在依据空的 `events` 列表下结论之前，务必先读 `sources`。每个数据源都会报告自己的 `state`——`ok`、`empty`、`stale`、`failed` 或 `unavailable`——只要其中任何一个不健康，`degraded` 即为 true，因此上游故障造成的沉默不会被误读为平静的一周。本接口上有两个数据源恒为 `unavailable`：军用船舶通道来自浏览器内的实时 AIS 流，服务端没有对应实现；中东打击追踪已于 2026 年 7 月退役。
