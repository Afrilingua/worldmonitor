...
结果包含 `scenarioId`、`scopedIso2`、`computedAt` 和 `coverage`。覆盖状态为 `complete`、`partial` 或 `unknown`；完整仅指清单内的种子范围。每条国家/HS2 记录区分 `evaluated`、`missing`、`malformed` 和 `not_seeded`。有效零影响保留为证据，但不高亮受影响国家；0% 物理关闭不高亮中断路线。关税影响仍按原公式计算。
