...
`summary.warn` is the complete actionable warning census. `summary.containedWarn` is a subset, not an additional bucket. Only `STALE_SEED`, `SEED_ERROR`, `STALE_CONTENT`, `COVERAGE_PARTIAL`, and `COVERAGE_DEGRADED` are eligible for containment, and only with metadata-proven positive records, no known unusability, and no active pending grace. Containment checks all required diagnostics independently of the selected status. Unknown freshness, incompatible reader policies or cache state, physical-divergence warnings, and `CHINA_DEGRADED` remain uncontained. Summary row counts do not prove that the underlying China datasets remain usable. A cohort of at most 3% keeps availability `HEALTHY`; missing or unusable data, unknown states, `REDIS_PARTIAL`, `ROLLOUT_PENDING`, broader impact, and critical failures do not.
