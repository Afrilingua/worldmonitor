...
- **Template echo** — the worker-derived template key (`affectedChokepointIds.join('+')`, or `tariff_shock` when there are no physical chokepoints), duration, disruption percent, and cost-shock multiplier so clients can render the run without re-looking up the catalog. The status result includes the selected country, effective severity and evaluated sector scope in `coverage`.
...
A missing, old or invalid seed manifest produces **unknown coverage**. The worker does not fall back to a guessed country list or scan Redis. It reads at most 100 exposure keys per batch. A complete result means complete within the bounded seed scope, not global trade coverage.
