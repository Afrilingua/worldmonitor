...
- SZSE endpoints are date-keyed, so the seeder pins the date from the exchange
  ...
  probes back a bounded number of trading days; and
...
exports of every reviewed heading: per heading, the observation year and the
reporters ranked by exported value, each with value in USD and net weight when
reported. The run reserves two requests for it against the request budget
