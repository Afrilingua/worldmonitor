...
The `comtrade:bilateral-hs4` seed also reports `coverage_partial` with `stale: false` when its last run preserved a country's previous payload, recorded a country as `unavailable`, `malformed`, `incomplete` or `not_attempted`, or failed the run-level world-exports fetch. Valid observations do not count: a reporter with no positive rows (`no_records`), or an importer that does not trade every reviewed heading. The entry carries a `bilateralCoverage` diagnostic with `preservedCountries` (ISO2 codes), `countryCoverage` (per-country `state`, `attemptedAt`, `missingHs4s` for observed or empty reporters, and `rowCounts` — the raw rows the provider returned for each catalogue batch, so a response trimmed at the row cap is distinguishable from a reporter that genuinely trades little; a country whose second batch never ran carries a single count), `productCoverageKnown`, which is `false` until the seeder has written per-country coverage, and `worldExports` — the outcome of the two requests the run reserves before any reporter to read every reporter's exports of every reviewed heading, carrying `state` (`observed`, `unavailable`, `malformed` or `incomplete`), `fetchedAt`, `headingCount` and `reporterCount`. Any `state` other than `observed` is a coverage gap, because the brief's supplier scale and world rank come from that one key; `worldExports` is `null` on a run recorded before the field existed, which is absence of evidence rather than a failure.
