...
The `comtrade:bilateral-hs4` seed also reports `coverage_partial` with `stale: false` when its last run preserved a country's previous payload, or recorded a country as `unavailable`, `malformed`, `incomplete` or `not_attempted`. Valid observations do not count: a reporter with no positive rows (`no_records`), or an importer that does not trade every reviewed heading. The entry carries a `bilateralCoverage` diagnostic with `preservedCountries` (ISO2 codes), `countryCoverage` (per-country `state`, `attemptedAt`, `missingHs4s` for observed or empty reporters, and `rowCounts` — the raw rows the provider returned for each catalogue batch, so a response trimmed at the row cap is distinguishable from a reporter that genuinely trades little; a country whose second batch never ran carries a single count), and `productCoverageKnown`, which is `false` until the seeder has written per-country coverage.
