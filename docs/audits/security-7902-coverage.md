...
| [#7901](https://github.com/koala73/worldmonitor/issues/7901) (OPEN) — seven npm lockfiles; Cargo; Umami pnpm | [7912](https://github.com/koala73/worldmonitor/pull/7912) | [PR #7920](https://github.com/koala73/worldmonitor/pull/7920) is open: Rust advisory gate for #5935, compatible crate repairs, dated npm/pnpm dispositions and 103 focused tests reported. The glib decision is proposed, not accepted; maintenance notices remain. Linux integration, CI and merge/release acceptance are separate. |

All ten listed PR heads are ancestors of the inventory revision. Their exact heads, changed files and current descriptions are retained in local `prs.json`. Test results above are prior PR reports; this lane did not rerun application tests. Closed child state does not dispose of an omitted lead or prove rollout. The later URL-object repair in #7919 is outside the inventory revision: its open head is `e535f31b42002992332709efc39e0c75622e7068`. Its reported regression proves the wrapper defect, not a public attacker-controlled production route.
...
- [ ] **Release acceptance lane / #7892 owner:** complete PR #7919 CI, separately authorized merge and release acceptance for the confirmed URL-object wrapper defect; its local repair evidence does not establish production reachability or rollout.
...
- [ ] **Dependencies/CI lane / #7901 and existing #5935 owner:** complete PR #7920 CI and Linux integration, obtain an explicit owner decision on the proposed glib treatment before its October 8 expiry, and verify the first main/daily advisory runs after separately authorized merge. Keep maintenance notices and database outages visible; do not call the warning result clean.
