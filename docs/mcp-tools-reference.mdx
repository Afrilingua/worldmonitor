...
Read `sources` before drawing any conclusion from an empty `events` list. Every producer reports its own `state` — `ok`, `empty`, `stale`, `failed` or `unavailable` — and `degraded` is true whenever any of them is not healthy, so silence caused by a dead upstream is never mistaken for a quiet week. Two producers are permanently `unavailable` on this surface: the military-vessel lane comes from a browser-held live AIS stream with no server-side equivalent, and Middle East strike tracking was retired in July 2026.
...
| `window_hours` | integer | no | Look-back window in hours, 1-168. Defaults to 168 (7 days), which is what the UI shows. The upstream coverage query is pinned to 7 days, so a wider value is rejected rather than silently returning the same events. |
| `limit` | integer | no | Maximum timeline events to return, keeping the most recent. 1-500, defaults to 200. |
