...
describe('ensureInlineTypedInput (fixture)', () => {
  ...
  it('shortens a long description on the restored copy and leaves the component whole', () => {
    const full = 'Optional JMESPath expression applied server-side (mirrors the MCP argument). '
      + 'Invalid expressions, expressions larger than 1024 UTF-8 bytes, or projections over the '
      + 'output cap return HTTP 400 with a {_jmespath_error, original_keys} envelope. '
      + 'Grammar and worked examples: https://example.test/docs.';
    const spec = {
      openapi: '3.1.0',
    ...
    assert.match(restored.description, /^Optional JMESPath expression applied server-side\./);
